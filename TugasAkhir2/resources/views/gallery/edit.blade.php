            ...
            <div class="col-12">
                ...
                <form action="{{ route('gallery.update', $items->id) }}" method="post" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <select name="produks_id" required class="form-control">
                            @foreach($product as $tampil)
                                <option value="{{ $tampil->id }}" {{old('produks_id') ?? $items->produks_id == $tampil->id ? 'selected' : ''}}>
                                    {{ $tampil->produk }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" @error('nama_gambar') is-invalid @enderror id="nama_gambar" name="nama_gambar" value="{{$items->nama_gambar}}">
                        @error('nama_gambar')
